...
function parseBlock(block: string): DictMeta | null {
  function extractField(fieldName: string): string | null {
    const re = new RegExp(`\\b${fieldName}\\s*:\\s*(['"])((?:\\\\.|[^\\\\])*?)\\1`)
...
function main() {
  ...
  while (i < source.length) {
    if (source[i] === '{') {
      ...
      if (/\bid\s*:/.test(block) && /\burl\s*:/.test(block) && /\blength\s*:/.test(block)) {
  ...
  console.log(`Extracted ${unique.length} dictionary metadata entries to ${OUTPUT_PATH}`)
  ...
  if (results.length > unique.length) {
    ...
    for (const [id, count] of Object.entries(ids)) {
      if (count > 1) console.warn(`  WARNING: duplicate id '${id}' (${count} times) — kept last`)
    }
  }
}
