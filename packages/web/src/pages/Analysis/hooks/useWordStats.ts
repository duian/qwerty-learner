import { fetchWordRecordsByTimeRange } from '@/api/record-api'
...
async function getChapterStats(startTimeStamp: number, endTimeStamp: number): Promise<IWordStats> {
  // Fetch word records within the timestamp range via API
  const records: IWordRecord[] = await fetchWordRecordsByTimeRange(startTimeStamp, endTimeStamp)
